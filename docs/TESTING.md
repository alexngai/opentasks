...
### Tier 2: Integration Tests (Proposed)
...
**Deliverables:**
- `tests/integration/helpers/` - shared utilities
- `vitest.integration.config.ts`
- Updated `package.json` scripts

### Phase 2: Storage Integration Tests (RUN_SLOW_TESTS)

**Priority: High** - Foundation for all persistence

1. Create JSONL durability tests
   - Crash recovery (partial writes, corrupted entries)
   - Compaction with 1000+ entries
   - Concurrent readers
2. Create SQLite durability tests
   - Transaction crash recovery
   - 10k+ node performance baselines
...
### Phase 3: Daemon Integration Tests (RUN_SLOW_TESTS)

**Priority: High** - Foundation for client-server architecture

1. Create daemon spawn/lifecycle helpers
2. Write lifecycle tests (startup, shutdown, signal handling)
3. Write IPC round-trip tests
4. Write concurrent client tests
5. Write flush-on-shutdown tests
...
### Phase 4: Provider Integration Tests (RUN_SLOW_TESTS)

**Priority: Medium** - External system integration

1. BeadsProvider integration (requires `bd` CLI)
   - Read/write operations against real Beads directory
   - Skip gracefully if `bd` not available
2. ClaudeTasksProvider file store
   - File-backed persistence tests

**Deliverables:**
- `tests/fixtures/beads/` - test fixtures
- `tests/integration/providers/beads.integration.test.ts`
- `tests/integration/providers/claude-tasks.integration.test.ts`
...
| Tier | Target | Current |
|------|--------|---------|
| Unit | >90% | ~85% (estimated) |
| Integration | >70% of external interfaces | 0% |
...
|-----------|--------|----------|
| Unit test suite | <10s | 2.8s ✅ |
| Integration test suite | <2min | TBD |
| E2E test suite | <10min | TBD |
| Create 1000 nodes | <5s | TBD |
| Query 10k nodes | <1s | TBD |

---

## Next Steps

1. **Immediate:** Implement Phase 1 (test infrastructure, helpers, config)
2. **Short-term:** Implement Phases 2-3 (storage + daemon integration tests)
