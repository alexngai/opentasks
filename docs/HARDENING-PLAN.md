...
- [ ] 5.2 git-cascade ingestion · 5.3 echo-loop audit · flagship demo — pending. **Design settled (Alex):** OpenTasks stays **git-cascade-agnostic** — no provider, no handler in core, no git-cascade runtime/dev coupling. The cascade→edge glue is **caller-side**: a consumer maps a cascade event's `task_ref` → existing `createNode(external cascade://…)` + `link(…, 'implements'/'references')`. 5.2 + the flagship demo merge into **one in-repo integration test** (demo scope = in-repo test, also Alex's call): a real opentasks daemon + **faithful `x-cascade/stream.*` event shapes** (the integration contract is the event format; a smoke showed driving git-cascade's real tracker has event/metadata quirks not worth coupling to) → create task → claim → map events to provenance edges → close → assert `task → stream → commit` is queryable in one call. The cascade-event→link mapping is documented as the caller-side pattern, not shipped in opentasks core. *Follow-up (noted, not now):* a `cascade://` provider only if a clean event/feed source emerges.
