...
- **Deferred — multi-location watch (fix 6a):** the multi-location daemon watches only the first-subscribed location; non-primary worktrees emit/buffer nothing and events lack a location discriminator. Scoped in [docs/MULTI-LOCATION-WATCH.md](./MULTI-LOCATION-WATCH.md) (≈P4). Single-location path is unaffected.
