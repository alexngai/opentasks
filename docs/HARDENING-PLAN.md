...
- [~] **4.7 INVESTIGATED (recommendation pending decision).** `init --global` works (creates `~/.opentasks`), and the global daemon is reachable via the client's `~/.opentasks` socket fallback **or** `OPENTASKS_PROJECT_DIR=~/.opentasks`. But "use-from-anywhere" is **broken by default**: from a random dir, a command auto-starts a *local* store at the cwd (resolveProjectDir is cwd-based) — never the global one. There is no first-class `--global` flag on commands. **Recommendation: make it work (cheaply)** — add a `--global` flag + `OPENTASKS_GLOBAL=1` env that routes `resolveProjectDir` → `~/.opentasks` (stripped in `main()` like `--no-autostart`), so auto-start + client agree on the global socket; local-by-default + explicit `--global` is the sane model and the plumbing is ~90% there. Alternative: cut the README section. **Awaiting Alex's make-vs-remove call.**
