...
describe('evaluateConditions', () => {
  ...
  it('matches branch condition', () => {
    ...
    expect(
      evaluateConditions({ branch: 'feature-*' }, { role: 'worker', branch: 'main' })
    ).toBe(false)
  })

  it('matches exact branch', () => {
    expect(
      evaluateConditions({ branch: 'main' }, { role: 'worker', branch: 'main' })
    ).toBe(true)
    expect(
      evaluateConditions({ branch: 'main' }, { role: 'worker', branch: 'develop' })
    ).toBe(false)
  })

  it('returns false when branch condition set but no branch in context', () => {
    expect(
      evaluateConditions({ branch: 'main' }, { role: 'worker' })
    ).toBe(false)
  })
