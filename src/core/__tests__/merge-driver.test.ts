...
describe('fieldLevelMerge', () => {
  it('takes agreed-upon value when both sides match', () => {
    ...
    const theirs = { id: 't-1', status: 'closed', title: 'Test', updated_at: '2025-01-01T02:00:00Z' }
  ...
  it('takes theirs when only theirs changed', () => {
    ...
    const theirs = { id: 't-1', status: 'in_progress', title: 'Test', updated_at: '2025-01-01T02:00:00Z' }
...
describe('mergeJsonl', () => {
  ...
  it('handles true conflict with field-level merge', () => {
    const base = writeJsonl('base.jsonl', [
      { id: 't-1', status: 'open', title: 'Original', priority: 1, updated_at: '2025-01-01T00:00:00Z' },
    ])
    const ours = writeJsonl('ours.jsonl', [
      { id: 't-1', status: 'closed', title: 'Original', priority: 1, updated_at: '2025-01-01T01:00:00Z' },
    ])
    const theirs = writeJsonl('theirs.jsonl', [
      { id: 't-1', status: 'open', title: 'Updated Title', priority: 2, updated_at: '2025-01-01T02:00:00Z' },
    ])
