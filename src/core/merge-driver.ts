...
export function parseJsonlToMap(filePath: string): Map<string, JsonLine> {
  ...
  for (const line of content.split('\n')) {
    ...
    try {
      ...
      if (
        !existing ||
        (obj.updated_at as string) > (existing.updated_at as string)
      ) {
        map.set(id, obj)
      }
    } catch {
 ...
 */
export function writeJsonlFromMap(
  filePath: string,
  entries: Map<string, JsonLine>
): void {
  const lines: string[] = []
...
export function fieldLevelMerge(
  ...
  const times = [ours.updated_at as string, theirs.updated_at as string].filter(
    Boolean
  )
  if (times.length > 0) {
...
export function installMergeDriver(worktreePath: string): void {
  ...
  try {
    ...
    execSync(
      'git config merge.opentasks.driver "opentasks merge-driver %O %A %B %L %P"',
      { cwd: worktreePath, stdio: ['pipe', 'pipe', 'pipe'] }
    )
  } catch {
