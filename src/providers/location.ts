 ...
 */
function toProviderNode(row: Record<string, unknown>, connection: Connection): ProviderNode {
  return {
    id: row.id as string,
  ...
  return {
    ...
    async get(id: string): Promise<ProviderNode | null> {
      try {
        ...
        const row = database.prepare('SELECT * FROM nodes WHERE id = ?').get(id) as
    ...
    async create(): Promise<ProviderNode> {
      throw new ProviderError(
        'NOT_SUPPORTED',
        'Location provider is read-only',
        `opentasks-${connection.hash}`,
      );
    },
    ...
    async ready(): Promise<ProviderNode[]> {
      try {
        const database = getDb();
        const rows = database.prepare('SELECT * FROM ready_tasks').all() as Record<
          string,
          unknown
        >[];
        return rows.map((r) => toProviderNode(r, connection));
...
export function initializeLocationProviders(
  ...
  for (const conn of connections) {
    const resolvedPath = basePath ? path.resolve(basePath, conn.path) : path.resolve(conn.path);
