...
const mockTaskProvider: Provider & TaskManageable = {
  ...
  readyTasks: async (options): Promise<ProviderNode[]> => {
    const tasks: ProviderNode[] = [
      { id: 'task-1', uri: 'tasks://task-1', type: 'issue', title: 'Ready 1', status: 'open', fetchedAt: new Date().toISOString() },
      { id: 'task-2', uri: 'tasks://task-2', type: 'issue', title: 'Ready 2', status: 'open', priority: 1, fetchedAt: new Date().toISOString() },
      { id: 'task-3', uri: 'tasks://task-3', type: 'issue', title: 'Ready 3', status: 'open', priority: 2, fetchedAt: new Date().toISOString() },
    ]
...
describe('TaskManageable implementation', () => {
  ...
  it('exposes task capabilities', () => {
    ...
    expect(mockTaskProvider.taskCapabilities.statusModel).toEqual(['open', 'in_progress', 'blocked', 'done'])
  })
})
