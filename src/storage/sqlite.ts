...
export class SQLitePersister implements Storage {
  ...
  async updateNode(id: string, updates: Partial<StoredNode>): Promise<void> {
    ...
    const updateableFields = [
      'title', 'content', 'content_hash', 'status', 'priority',
      'assignee', 'parent_id', 'source', 'archived', 'updated_at',
      'target_id', 'feedback_type', 'thread_id', 'reply_to_id',
      'resolved', 'dismissed', 'uri', 'materialized', 'cached_at',
      'stale', 'external_status', 'claimed_by', 'claimed_at',
      'lock_until', 'location', 'branch',
    ]
