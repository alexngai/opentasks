...
describe('CLI Tool Commands', () => {
  ...
  describe('cmdLink', () => {
    ...
    it('should error on missing --from', async () => {
      await expect(cmdLink(['--to', 'c-test1', '--type', 'implements'])).rejects.toThrow(
        'process.exit(1)',
      );
    ...
    it('should error on missing --to', async () => {
      await expect(cmdLink(['--from', 't-test1', '--type', 'implements'])).rejects.toThrow(
        'process.exit(1)',
      );
    ...
    it('should error on missing --type', async () => {
      await expect(cmdLink(['--from', 't-test1', '--to', 'c-test1'])).rejects.toThrow(
        'process.exit(1)',
      );
  ...
  describe('cmdAnnotate', () => {
    ...
    it('should create feedback with fromId', async () => {
      ...
      await cmdAnnotate([
        '{"targetId":"c-1","fromId":"t-1","create":{"content":"Done","type":"comment"}}',
      ]);
    ...
    it('should create anchored suggestion', async () => {
      ...
      await cmdAnnotate([
        '{"targetId":"c-1","create":{"content":"Add rate limiting","type":"suggestion","anchor":{"line":15}}}',
      ]);
  ...
  describe('cmdCreate', () => {
    ...
    it('should error on missing --title', async () => {
      await expect(cmdCreate(['--type', 'task'])).rejects.toThrow('process.exit(1)');
  ...
  describe('cmdDelete', () => {
    ...
    it('should handle RPC errors', async () => {
      ...
      await expect(cmdDelete(['t-nonexistent'])).rejects.toThrow('process.exit(1)');
