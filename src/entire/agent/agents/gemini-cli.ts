  ...
  implements Agent, HookSupport, TranscriptAnalyzer, TokenCalculator, TranscriptChunker
{
  ...
  async installHooks(repoPath: string, force = false): Promise<number> {
    ...
    const hookDefs = [
      ...
      { key: 'SessionEnd', hookName: 'session-end', matcher: 'exit', name: 'entire-session-end-exit' },
      { key: 'SessionEnd', hookName: 'session-end', matcher: 'logout', name: 'entire-session-end-logout' },
      ...
      { key: 'BeforeToolSelection', hookName: 'before-tool-selection', matcher: '', name: 'entire-before-tool-selection' },
