...
export async function enable(options: EnableOptions = {}): Promise<EnableResult> {
  ...
  if (!(await isGitRepository(cwd))) {
    return { enabled: false, agent: null, agentHooksInstalled: 0, gitHooksInstalled: 0, errors: ['Not a git repository'] };
  }
  ...
  try {
    ...
    return { enabled: false, agent: null, agentHooksInstalled: 0, gitHooksInstalled: 0, errors: ['Repository has no commits'] };
  }
