...
export function createFileWatcher(config: WatcherConfig): FileWatcher {
  const { locationPath, debounceMs = DEFAULT_DEBOUNCE_MS, watchMarkdown = true } = config;
  ...
  return {
    ...
    async start(): Promise<void> {
      ...
      if (watchMarkdown) {
        watchPaths.push(path.join(locationPath, 'context'), path.join(locationPath, 'tasks'));
      }
