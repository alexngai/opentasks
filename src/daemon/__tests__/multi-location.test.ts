...
import { createDaemon, type Daemon, type MultiLocationDaemonConfig } from '../lifecycle.js';
...
vi.mock('../location-state.js', async (importOriginal) => {
  ...
  return {
    ...actual,
    createLocationState: vi
      .fn()
      .mockImplementation(async (opentasksPath: string, hash: string, primary: boolean = false) => {
...
function createMockLocationState(
  hash: string,
...
describe('Multi-Location Daemon', () => {
  ...
  function createMultiConfig(
    overrides: Partial<MultiLocationDaemonConfig> = {},
