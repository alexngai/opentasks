...
export function registerArchiveMethods(options: ArchiveMethodsOptions): void {
  ...
  server.handle<ArchiveNodeParams, ArchiveEventResult>('archive.node', async (params) => {
    const { uri, location } = params || {};
    if (!uri) {
      throw new Error('Missing required parameter: uri');
    }
    ...
    if (!state.archiver) {
      return {
        uri,
        stores: [{ storeName: 'none', stored: false, error: 'Archive functionality not enabled' }],
      };
    }
  ...
  server.handle<
    ArchiveStatusParams,
    { enabled: boolean; stores: Array<{ name: string; healthy: boolean; message?: string }> }
  >('archive.status', async (params) => {
    const { location } = params || {};
    const state = locationResolver.resolve(location);
