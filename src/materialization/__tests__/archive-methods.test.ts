...
function createMockLocationState(withArchiver: boolean): LocationState {
  const mockArchiver = withArchiver
    ? {
        onSessionEvent: vi
          .fn()
          .mockResolvedValue({ uri: 'test://uri', stores: [{ storeName: 'git', stored: true }] }),
        archiveNode: vi
          .fn()
          .mockResolvedValue({ uri: 'test://uri', stores: [{ storeName: 'git', stored: true }] }),
        ...
        rematerializeAll: vi
          .fn()
          .mockResolvedValue({ restored: 2, failed: 0, skipped: 1, errors: [] }),
        listArchived: vi
          .fn()
          .mockResolvedValue([
            {
...
function createMockResolver(state: LocationState): LocationResolver {
  return {
    ...
    getDefault: () => state,
    list: () => [
      { hash: state.hash, opentasksPath: state.opentasksPath, primary: true, healthy: true },
    ],
    has: () => true,
    add: vi.fn(),
    remove: vi.fn(),
