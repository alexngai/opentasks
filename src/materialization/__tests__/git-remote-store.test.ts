...
import type { RemoteStoreConfig, MaterializationSnapshot, SessionSnapshot, CheckpointSnapshot } from '../types.js'
...
describe('Git Remote Store', () => {
  describe('creation', () => {
    ...
    it('should throw if URL is missing', () => {
      expect(() => createGitRemoteStore(makeConfig({
        config: {},
      }))).toThrow('url')
    })
  })
  ...
  describe('archive', () => {
    ...
    it('should not push on on-close policy until close', async () => {
      ...
      const remoteBranchesAfter = execSync(
        `git -C "${remoteRepoPath}" branch`,
        { encoding: 'utf-8' }
      )
  ...
  describe('retrieve', () => {
    ...
    it('should fetch from remote before reading when fetchBeforeRead is enabled', async () => {
      ...
      const store2 = createGitRemoteStore(makeConfig({
        config: {
          url: remoteRepoPath,
  ...
  describe('conflict resolution', () => {
    it('should handle concurrent pushes from different caches', async () => {
      ...
      const store1 = createGitRemoteStore(makeConfig({
        name: 'store-1',
