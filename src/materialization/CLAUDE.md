...
- `http-remote-store.ts` — `createHttpRemoteStore()`; POSTs snapshots to HTTP endpoints
...
The HTTP store is write-only (`retrieve()` returns null, `list()` returns `[]`). New stores can implement reads if the backend supports it.
...
Paths are constructed in `git-archive-store.ts`. The pattern is `<graphId>/sessions/<session-id>/session.json` (and `edges.json`, `checkpoints/<id>.json`). Changing this requires a migration strategy for existing archives.
