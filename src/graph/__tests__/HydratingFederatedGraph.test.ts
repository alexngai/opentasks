...
describe('HydratingFederatedGraph', () => {
  ...
  beforeEach(() => {
    ...
    mockProvider = {
      ...
      supportedEdgeTypes: vi
        .fn()
        .mockReturnValue([{ type: 'blocks', canQuery: true, canCreate: true, canDelete: true }]),
    };
  ...
  describe('hydrate()', () => {
    ...
    it('re-hydrates stale nodes', async () => {
      ...
      (mockProvider.parseUri as Mock).mockReturnValue({
        scheme: 'beads',
        id: 'bd-stale',
        isRelative: true,
      });
  ...
  describe('resolveAll()', () => {
    it('resolves multiple URIs', async () => {
      ...
      (mockProvider.get as Mock).mockResolvedValueOnce(node1).mockResolvedValueOnce(node2);
      (mockProvider.parseUri as Mock).mockImplementation((uri: string) => {
