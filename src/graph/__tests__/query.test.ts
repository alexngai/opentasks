...
function createEdge(id: string, fromId: string, toId: string, type: EdgeType): StoredEdge {
  return {
    id,
    uuid: `uuid-${id}`,
    from_id: fromId,
...
describe('QueryEngine', () => {
  ...
  describe('blockers()', () => {
    ...
    it('should get transitive blockers', async () => {
      ...
      vi.mocked(storage.getNode).mockResolvedValueOnce(blockerA).mockResolvedValueOnce(blockerB);
  ...
  describe('parent()', () => {
    it('should get parent node', async () => {
      ...
      vi.mocked(storage.getNode).mockResolvedValueOnce(child).mockResolvedValueOnce(parent);
