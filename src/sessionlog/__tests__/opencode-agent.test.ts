  ...
  describe('extractAllUserPrompts', () => {
    it('should extract user prompts from session', () => {
      const data = JSON.stringify({
        info: { id: 'sess-1' },
        messages: [
          { info: { id: 'msg-1', role: 'user', time: { created: 1 } }, parts: [{ type: 'text', text: 'First' }] },
          { info: { id: 'msg-2', role: 'assistant', time: { created: 2 } }, parts: [{ type: 'text', text: 'Response' }] },
          { info: { id: 'msg-3', role: 'user', time: { created: 3 } }, parts: [{ type: 'text', text: 'Second' }] },
        ],
      });
