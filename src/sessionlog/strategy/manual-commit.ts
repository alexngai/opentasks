...
export function createManualCommitStrategy(
  ...
  async function findSessionsForCommit(
  ...
  const strategy: Strategy = {
    ...
    async prepareCommitMsg(
      ...
      for (const state of sessions) {
        ...
        const shadowBranch = getShadowBranchName(
          state.baseCommit,
          state.worktreeID,
        );
    ...
    async postCommit(): Promise<void> {
      ...
      const parentHash = await gitSafe(
        ['rev-parse', `${headHash}^`],
        { cwd },
      );
      ...
      const { added, modified, deleted } = await diffNameOnly(
        `${headHash}^`,
        headHash,
        cwd,
      );
    ...
    async saveTaskStep(step: TaskStepContext): Promise<void> {
      ...
      const commitMsg = formatShadowCommit(
        messageSubject,
        metadataDir,
        step.sessionID,
      );
  ...
  async function getLogsOnlyRewindPoints(
    limit: number,
  ): Promise<RewindPoint[]> {
 ...
 */
function injectCheckpointTrailer(
  message: string,
  trailer: string,
): string {
